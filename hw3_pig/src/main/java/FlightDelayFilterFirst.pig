...
flight2 = FOREACH data2 GENERATE $0 AS year:int,
                                 ...
                                 $41 AS cancelled:int,
                                 $43 AS diverted:int;
-- filter out cancelled, diverted, out of date-range flights and flights with incorrect orgins/destinatinos
flight1 = FILTER flight1 BY (cancelled == 0 AND diverted == 0 AND origin == 'ORD' AND dest != 'JFK' AND
            ((year == 2007 AND month >= 6) OR (year == 2008 AND month <= 5))
            );
flight2 = FILTER flight2 BY (cancelled == 0 AND diverted == 0 AND origin != 'ORD' AND dest == 'JFK' AND
            ((year == 2007 AND month >= 6) OR (year == 2008 AND month <= 5))
            );
...
delay_min = FOREACH joined_flights GENERATE (flight1::arrDelayMin + flight2::arrDelayMin) AS total_delay;
